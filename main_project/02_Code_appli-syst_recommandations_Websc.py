...
#On converti les chaines en listes, on associe les acteurs et leurs rangs, on les trie, on vérifie que l'acteur rang 1 correspond afin de conserver la ligne
        ...
        dico_eval = {}
        liste_g = genres_str.split(',')
        for i in range(len(df_cinema)):
            count = 0 
            for j in ast.literal_eval(df_cinema['Genre_split'][i]):
                for k in liste_g: 
                    if k == j : 
                        count +=1
                        dico_eval[df_cinema['Nom'][i]] = count
        c = 0        
        ...
        df_film = df_cinema[df_cinema['Nom'] == k]
        poster_1 = df_film['Poster'].values[0]
        resum = df_film['Résumé'].values[0]
        dur = df_film['Durée_minutes'].values[0]
        genre = df_film['Genre'].values[0]
        cast = df_film['Casting'].values[0]
        real =df_film['Réalisateur'].values[0]
        if k != '':
            st.subheader("Actuellement au cinema le Senechal")            
            st.image(poster_1, width=300)
            st.write(f'titre : {k}') 
            st.write(f'synopsys: {resum}')  
            st.write(f'durée : {dur} mn')
            st.write(f'genres : {genre}') 
            st.write(f'casting : {cast}')
            st.write(f'réalisateur : {real}')
